...
export const ChatListItem = ({item, handleComponents, click, active}: Props) => {
    
  return (
    <div onClick={() => click(item)} className={`${active ? 'bg-gray-100' : ''} flex cursor-pointer items-center gap-2 w-full border-b p-2 border-gray-300 hover:bg-gray-50`}>
        ...
        <div className="w-full flex overflow-hidden flex-col justify-center"> {/*chatListItem-lines */}
            ...
            > {/*chatListItem-line */}
                <div className="text-sm text-black">{item.name}</div> {/*chatListItem-name */}
